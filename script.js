document.addEventListener('DOMContentLoaded', async function () {
    ...
    function displayData(data) {
        ...
        const tableRowTitles = ["Avatar", "Name", "Rank", "Points", "Give Respect"];
        ...
        async function displayChunk(chunkIndex, chunkSize) {
            ...
            await Promise.all(chunkData.map(async function (user) {
                ...
                const dataGroup = [user.profile.avatar, user.profile.name, user.profile.rank, user.profile.points]
                ...
                if (!user.profile.id || user.profile.id === undefined) {
                    ...
                    for (let index = 0; index < 5; index++) {
                        ...
                        if (index === 0) {
                            ...
                            avatarImage.src = `https://www.hackthebox.com${user.profile.avatar}`;
                            avatarImage.alt = 'Avatar';
                            avatarImage.classList.add('avatar-image');
                            tableData.appendChild(avatarImage);
                        } else if (index === 4) {
